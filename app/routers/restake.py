...
from app.services.rust_client import confirm_restake_on_chain
...
async def initiate_restake(request: RestakeRequest):
    async with async_session() as session:
        try:
            ...
            return {"message": "Restake initiated", "operation_id": new_restake.operation_id}
...
@router.get("/check_restake/{operation_id}")
async def check_restake_status(operation_id: int):
    async with async_session() as session:
        try:
            # Query the database for the specific restake operation
            operation_query = select(RestakeOperation).where(RestakeOperation.operation_id == operation_id)
            ...
            return {"operation_id": operation.operation_id, "status": operation.status, "amount": operation.amount}
...
async def confirm_restake(request: ConfirmRequest):
    async with async_session() as session:
        try:
            operation_query = select(RestakeOperation).where(RestakeOperation.operation_id == request.operation_id)
            result = await session.execute(operation_query)
            operation = result.scalar_one_or_none()
            ...
            response = await confirm_restake_on_chain(request.operation_id, request.amount)
            if response is None or response.status_code != 200:
                raise HTTPException(status_code=500, detail="Failed to confirm restake on blockchain")
